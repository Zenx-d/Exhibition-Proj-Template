...
function isGibberish(localPart) {
  // 1. Check for long consonant clusters (hard to pronounce, typical of mashes)
  const consonants = localPart.match(/[^aeiouy0-9._%+-]{6,}/gi);
  if (consonants) return true;

  // 2. Check for lack of vowels in long strings
  if (localPart.length > 8) {
    const vowelCount = (localPart.match(/[aeiouy]/gi) || []).length;
    if (vowelCount === 0) return true;
    // Vowel ratio too low (< 10% for very long strings)
    if (vowelCount / localPart.length < 0.1) return true;
  }

  // 3. Repeated characters (e.g., aaaaaa@...)
  if (/(.)\1{6,}/.test(localPart)) return true;
...
export function validateEmail(email) {
  ...
  // 1. Format check using dedicated email-validator package
  const isFormat = EmailValidator.validate(trimmed);
