...
/**
 * Detect if the local part of an email is likely gibberish/keyboard mash.
 */
function isGibberish(localPart) {
  // 1. Check for common keyboard sequences
  const sequences = [
    'asdf', 'sdfg', 'dfgh', 'fghj', 'ghjk', 'hjkl', 
    'qwerty', 'qwert', 'werty', 'ertyu', 'rtyui',
    'zxcv', 'xcvb', 'vbnm',
    '1234', '2345', '3456', '4567', '5678', '6789',
    'qaz', 'wsx', 'edc', 'rfv', 'tgb', 'yhn', 'ujm', 'ik', 'ol'
  ];
  if (sequences.some(seq => localPart.includes(seq))) return true;

  // 2. Check for long consonant clusters (hard to pronounce, typical of mashes)
  // Non-vowel characters. Reduced threshold to 4 to catch mashes like 'lsdk'
  const consonants = localPart.match(/[^aeiouy0-9._%+-]{4,}/gi);
  if (consonants) return true;

  // 3. Check for lack of vowels in long strings
  if (localPart.length > 5) {
    const vowelCount = (localPart.match(/[aeiouy]/gi) || []).length;
    if (vowelCount === 0) return true;
    // Vowel ratio too low (< 15% for long strings)
    if (vowelCount / localPart.length < 0.15) return true;
  }
...
export function validateEmail(email) {
  ...
  if (isGibberish(localPart)) {
    return { 
      valid: false, 
      error: 'This email looks like a temporary or fake address. Please use your real email.', 
      suggestion: null 
    };
  }
  ...
  const suggestion = COMMON_DOMAIN_TYPOS[domain]
    ? trimmed.replace(domain, COMMON_DOMAIN_TYPOS[domain])
    : null;
  ...
  // 5. Basic TLD sanity (must be >= 2 chars)
