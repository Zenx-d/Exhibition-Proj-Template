...
function getDb() {
  ...
  if (!url) {
    console.warn('[Telemetry] Database URL missing!');
    return null;
  }

  console.log('[Telemetry] Creating new database connection pool');
  cachedSql = neon(url);
  return cachedSql;
}
...
// Whitelist common domains to prevent false negatives during DNS hiccups
const COMMON_DOMAINS = new Set(['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'icloud.com', 'protonmail.com', 'me.com']);

async function verifyEmailDomain(email) {
  const domain = email.split('@')[1]?.toLowerCase();
  if (!domain) return false;

  // Always allow common domains
  if (COMMON_DOMAINS.has(domain)) return true;

  try {
    const mxRecords = await resolveMx(domain);
    const isValid = mxRecords && mxRecords.length > 0;
    if (!isValid) console.warn(`[Telemetry] No MX records found for ${domain}`);
    return isValid;
  } catch (error) {
    console.error(`[Telemetry] DNS Lookup failed for ${domain}:`, error.message);
    // If DNS fails (e.g. offline/timeout), we allow it to pass to avoid blocking users
    return true;
  }
}
...
export async function logReferral(tag, path, source = 'unknown', metadata = {}) {
  ...
  // Rate limit referrals: 10 per minute per IP
  const ip = (await headers()).get('x-forwarded-for') || 'unknown';
  const { success: limitSuccess } = rateLimit(`referral_${ip}`, 10, 60000);
  if (!limitSuccess) {
    console.warn(`${logPrefix} Rate limit hit for ${ip}`);
...
export async function subscribeUser(email, source) {
  ...
  try {
    ...
    if (result.length === 0) {
      console.log(`[Telemetry] User already exists in DB: ${email}`);
      return { success: false, error: 'You are already subscribed to our newsletter!' };
    }

    console.log(`[Telemetry] Successfully saved new subscriber: ${email}`);
    return { success: true };
  } catch (error) {
    console.error('[Telemetry] CRITICAL Subscription Error:', error.message);
