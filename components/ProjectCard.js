...
export default function ProjectCard({ project, projectMembers }) {
  ...
  return (
    <div className="group relative flex flex-col bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-700 hover:-translate-y-2">
      {/* Thumbnail */}
      <Link href={`/projects/${id}`} className="block relative aspect-video bg-slate-100 dark:bg-slate-800 overflow-hidden">
        {thumbnail ? (
      ...
      <div className="p-8 flex flex-col flex-grow">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-slate-100 dark:border-slate-800">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href={`/projects/${id}`} className="flex items-center gap-1.5 font-black text-indigo-600 dark:text-indigo-400 text-[10px] uppercase tracking-widest group-hover:translate-x-1 transition-transform duration-500">
